...
public:
  ...
  void processImage(map<int, Eigen::Matrix<double, 7, 1>> &image,
  ...
  void predict(double t, const Vector3d &linearAcceleration,
               const Vector3d &angularVelocity);
  ...
  double reprojectionError3D(Matrix3d &Ri, Vector3d &Pi, Matrix3d &rici,
                             Vector3d &tici, Matrix3d &Rj, Vector3d &Pj,
                             Matrix3d &ricj, Vector3d &ticj, double depth,
                             Vector3d &uvi, Vector3d &uvj);
  ...
  bool getIMUInterval(
      double t0, double t1,
      std::vector<pair<double, pair<Eigen::Vector3d, Eigen::Vector3d>>>
          &imu_vector);
