...
bool MotionEstimator::solveRelativeRT_PNP(
    const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation,
    Vector3d &Translation) {
  ...
  for (int i = 0; i < int(corres.size()); i++) {
    if (corres[i].first(2) > 0 && corres[i].second(2) > 0) {
      lll.push_back(cv::Point3f(corres[i].first(0), corres[i].first(1),
                                corres[i].first(2)));
  ...
  cv::solvePnPRansac(lll, rr, cameraMatrix, cv::Mat(), rvec, tvec, false, 100,
                     1.0 / 460, 0.99, inliersArr,
                     cv::SOLVEPNP_EPNP); // maybe don't need 100times

  Vector3d tran(tvec.at<double>(0, 0), tvec.at<double>(1, 0),
                tvec.at<double>(2, 0));
  Matrix3d rota = SO3<double>::exp(Eigen::Vector3d(rvec.at<double>(0, 0),
                                                   rvec.at<double>(1, 0),
                                                   rvec.at<double>(2, 0)))
                      .matrix();
